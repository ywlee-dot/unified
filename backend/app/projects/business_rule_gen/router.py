...
WEBHOOK_URL = "http://n8n:5678/webhook/business_rule_gen"
PROJECT_SLUG = "business-rule-gen"
WORKFLOW_ID = "business-rule-gen-main"
WORKFLOW_NAME = "업무규칙 자동 생성 실행"


@router.post("/trigger/{workflow_id}", summary="업무규칙 생성 실행 (파일 업로드)")
async def trigger_workflow(
    workflow_id: str,
    file: UploadFile = File(...),
    db: AsyncSession = Depends(get_db),
):
    """컬럼정의서(Excel)를 업로드하면 n8n 워크플로우를 트리거합니다."""
    ...
    # 1) n8n 웹훅 호출 (multipart/form-data)
    try:
