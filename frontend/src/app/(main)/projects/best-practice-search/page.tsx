...
import { Upload, Play, Loader2, CheckCircle, XCircle, Download, Sparkles } from 'lucide-react';
...
export default function BestPracticeSearchPage() {
  ...
  const pollStatus = (runId: string) => {
    pollingRef.current = setInterval(async () => {
      try {
        const res = await fetch(`/api/projects/best-practice-search/runs/${runId}`, { credentials: 'include' });
        if (!res.ok) return;
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/projects/best-practice-search/trigger/best-practice-search-main', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
    ...
    <div className="p-6 max-w-2xl">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2.5 mb-1">
          ...
          >
            <Sparkles className="h-4 w-4" style={{ color: '#3182f6' }} />
          </div>
        ...
        <p className="mt-1 text-sm" style={{ color: '#8B95A1' }}>
          우수사례 데이터를 업로드하고 검색 워크플로를 실행하세요
        </p>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <Upload className="h-5 w-5" style={{ color: '#3182f6' }} />
          </div>
          ...
          {file && (
            <span
              ...
              style={{ backgroundColor: '#E8F1FF', color: '#3182f6' }}
            >
              선택됨
            </span>
          )}
