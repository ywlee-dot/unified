...
export default function Test1Page() {
  ...
  const pollStatus = (runId: string) => {
    pollingRef.current = setInterval(async () => {
      try {
        const res = await fetch(`/api/projects/data-quality-pretest/runs/${runId}`, { credentials: 'include' });
        if (!res.ok) return;
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/projects/data-quality-pretest/trigger/data-quality-pretest-main', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
