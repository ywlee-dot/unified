...
function BranchingPipeline({ graph, color }: { graph: PipelineGraph; color: string }) {
  const MIN_W = 100;
  const NODE_H = 68;
  const GAP_X = 32;
  const GAP_Y = 48;
  ...
  // Per-column widths (max across rows)
  ...
  // Cumulative column left positions
  ...
  // Determine which port side an edge uses on source/target
  ...
  // Track all edges per node+port for offset distribution
  ...
  // Compute edge endpoint coordinates
  ...
  const computeEdge = (edge: PipelineGraphEdge, i: number) => {
    ...
    // Corner-to-corner diagonal (right→top), adjusted for border-radius
    if (edge.srcPort === "right" && edge.tgtPort === "top") {
      const R = 12; // rounded-xl = 0.75rem
    ...
    // Auto straight edges (same-row horizontal, same-column vertical): no offset
